...
def obtener_datos_meteo(estaciones_ica):
    ...
    # 1. Inicializamos la estructura base del GeoJSON crudo
    mock_crudo_total = {
    ...
    for indice, chunk in enumerate(chunks):
        ...
        try:
            ...
            # 2. Acumulamos las features crudas de este lote en nuestro objeto global
            if 'features' in data:
                mock_crudo_total['features'].extend(data['features'])
    ...
    # 3. Guardamos TODO el historial crudo acumulado en un solo archivo al final
    try:
        with open("mock_meteo_raw.json", "w", encoding="utf-8") as f:
            json.dump(mock_crudo_total, f, ensure_ascii=False, indent=4)
        print("Mock crudo guardado correctamente con todas las features.")
    except IOError as e:
        print(f"Error al guardar el archivo mock: {e}")
...
if primeros:
    ...
    with open("mock_meteo.json", "w") as f:
